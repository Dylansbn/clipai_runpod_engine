...
def transcribe_with_whisper(video_path: str) -> Dict[str, Any]:
    """
    Utilise Whisper API officielle, compatible RunPod.
    """
...
def select_viral_segments(
    ...
    system_prompt = (
        ...
        f"Chaque extrait doit durer entre {min_duration} et {max_duration} secondes. "
...
Tâche :
- Choisir les {num_clips} extraits les plus viraux
- Durée entre {min_duration} et {max_duration}
- Structure du JSON :

{{
  "clips": [
    {{"start": 12.5, "end": 34.0, "title": "Titre", "reason": "Pourquoi"}}
  ]
}}
Réponds en {language}.
"""
    ...
    # ✔️ NOUVELLE SYNTAXE API OPENAI 2025
    raw = completion.choices[0].message.content
    print("🔎 JSON IA (début) :", raw[:300])
...
def generate_shorts(input_video_path: str, num_clips: int = 8, min_duration: float = 20, max_duration: float = 45):
    ...
    print("🚀 Lancement pipeline IA sur :", video)

    # 1) 🔊 Transcription Whisper API
    transcription = transcribe_with_whisper(str(video))
    segments = transcription["segments"]

    # 2) 🤖 Sélection des extraits viraux
    ...
    for i, clip in enumerate(viral, start=1):
        ...
        print(f"▶️ Clip {i} | {clip['start']}s → {clip['end']}s")
